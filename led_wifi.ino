...
SoftwareSerial modulo(2,3); // Pin 2 receptor azul, Pin 3 transmisor TX verde
const int sPIR = 8;
int mov = 0;
...
{
  Serial.begin(9600);
  modulo.begin(9600); // velocidad de transmisión podria ser diferente aunque genralmente es la que usa el el modulo 
  ...
  sendData("AT+CWMODE=3\r\n",1000,DEBUG); // configure el modulo con AP y Estación al mismo tiempo
   //AT+CIOBAUD=9600
  // AT+CWJAP="CompInt","lucas619"
  //sendData("AT+CWQAP\r\n",2000,DEBUG); // Eliminar la ultima conexion internet 
  //sendData("AT+CWLAP\r\n",3000,DEBUG);
  sendData("AT+CWJAP='Izzinet19979499','1972DE1970'\r\n",3000,DEBUG);// cambiar estos datos por los de tu red
...
int connectionId = modulo.read()-48; 
     ...
     modulo.find("pin="); // avanza el cursor a "sal="
